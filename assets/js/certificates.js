...
function loadCertConfig() {
  try {
    ...
    // Actualizar botón de abrir hoja
...
async function createNewSheet() {
  ...
  try {
    ...
    // ✅ Preparar body con token de autenticación (en body para evitar preflight CORS)
    let requestBody = {
      action: 'createSheet',
      params: {
        name,
    ...
    // ✅ Agregar token al body en lugar del header para evitar preflight CORS
    try {
      const idToken = await getFirebaseIdToken(true);
    ...
    if (data.success) {
      ...
      validatePDFGeneration();
      validateLinksGeneration();
      updateOpenSheetButton(); // Mostrar botón de abrir hoja
    } else {
      throw new Error(data.error || 'Error desconocido');
    }
  } catch (error) {
...
async function generatePDFs() {
  ...
  try {
    ...
    if (data.success) {
      if (typeof recordGenerationHistory === 'function') {
        recordGenerationHistory('PDFs', `Se procesaron certificados para ${data.total || 'la lista de'} alumnos.`);
      }
...
async function generateLinks() {
  ...
  try {
    ...
    if (data.success) {
      if (typeof recordGenerationHistory === 'function') {
        recordGenerationHistory('Enlaces', `Se generaron enlaces de descarga para ${data.total || 'la lista de'} alumnos.`);
      }
      // ✅ Ocultar el spinner cuando termine exitosamente
      if (spinnerEl) {
        spinnerEl.style.display = 'none';
...
// ===== FUNCIONES PARA ABRIR HOJA DE CÁLCULO =====

// Actualizar visibilidad del botón "Abrir hoja"
function updateOpenSheetButton() {
  const select = $('#select-google-sheet');
  const btnOpen = $('#btn-open-sheet');

  if (!select || !btnOpen) return;
...
// Abrir hoja de cálculo en Google Sheets
function openGoogleSheet() {
  const select = $('#select-google-sheet');
  if (!select || !select.value) {
    showToast('warning', 'No hay hoja seleccionada', 'Selecciona una hoja primero');
    return;
  }

  const sheetId = select.value.trim();
  if (!sheetId) {
    showToast('warning', 'No hay hoja seleccionada', 'Selecciona una hoja primero');
    return;
  }

  // URL de Google Sheets: https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit
  const sheetUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/edit`;
  console.log('[CERT] 🔗 Abriendo hoja:', sheetUrl);

  // Abrir en nueva pestaña
  window.open(sheetUrl, '_blank');

  showToast('success', 'Abriendo hoja', 'La hoja se abrirá en una nueva pestaña');
}
...
function setupCertificatesListeners() {
  ...
  // ✅ El event listener para "Ver Código del Script" está en index.html (línea 2239)
    ...
    'input-script-web-app-url'].forEach(id => {
      ...
      if (el) {
        el.addEventListener('change', () => {
          ...
          validatePDFGeneration();
          validateLinksGeneration();
          // Mostrar/ocultar botón de abrir hoja
          if (id === 'select-google-sheet') {
            updateOpenSheetButton();
          }
        });
      }
  ...
  // Botón para abrir hoja de cálculo
  $('#btn-open-sheet')?.addEventListener('click', openGoogleSheet);

  // Actualizar botón de abrir hoja al cargar configuración
  updateOpenSheetButton();
